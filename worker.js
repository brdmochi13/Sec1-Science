...
function doAuth() {
  ...
  console.log('doAuth() called — mode:', authMode, 'email:', email, 'USE_FIREBASE:', USE_FIREBASE);

  if (USE_FIREBASE) {
    if (!auth) { showErr('Firebase Auth not initialised. Refresh the page.', 'no-auth'); return; }
    if (authMode === 'login') {
      auth.signInWithEmailAndPassword(email, pwd)
        .then(function(uc) {
          console.log('✅ signIn OK, UID:', uc.user.uid);
          currentUser = uc.user;
          db.collection('users').doc(uc.user.uid).get()
            ...
            .catch(function(e) {
              console.warn('Firestore role fetch failed:', e.message, '— defaulting to student');
              currentRole = 'student';
            })
        ...
        .catch(function(e) {
          ...
          if (e.code === 'auth/user-not-found' || e.code === 'auth/invalid-credential')
            ...
            msg = 'Account temporarily locked — too many failed attempts. Wait 5 minutes or reset your password.';
...
,
    {
      ...
      secA:[
        {q:'Which part of the cell contains DNA, the hereditary material passed from parents to offspring?',opts:['Cell membrane','Vacuole','Cytoplasm','Nucleus'],ans:3,skill:'Cells',fb:'The nucleus contains DNA — the genetic/hereditary material passed from parents to offspring.'},
        {q:'A plant cell placed in salty water has its large central vacuole shrink. What does this tell you about the vacuole?',opts:['It produces energy for the cell','It stores water and can lose or gain water depending on conditions','It controls what enters the cell','It carries out photosynthesis'],ans:1,skill:'Cells',fb:'The vacuole stores water and food and waste materials. In salty water, water leaves the vacuole causing it to shrink.'},
        {q:'Robert Hooke first used the word cell after observing cork under a microscope. What Latin word does cell come from?',opts:['Cella (small room)','Cellula (little unit)','Cellum (enclosure)','Cello (hollow)'],ans:0,skill:'Cells',fb:'The word cell comes from the Latin word cella meaning small room. Hooke saw cork that looked like a honeycomb of tiny rooms.'},
        {q:'A scientist finds a single-celled organism with a cell membrane, cytoplasm and nucleus but NO cell wall and NO chloroplasts. What can she conclude?',opts:['It is a plant cell','It is an animal cell','It could be either','It is a fungus'],ans:1,skill:'Cells',fb:'No cell wall and no chloroplasts are both animal cell characteristics. This organism fits the typical animal cell model.'},
        {q:'What does the cell membrane do that the cell wall cannot?',opts:['Provide rigid structural support','Control which substances enter and leave the cell','Give the cell a fixed regular shape','Protect the cell from bursting'],ans:1,skill:'Cells',fb:'The cell membrane is PARTIALLY PERMEABLE and selectively controls what enters and leaves. The cell wall is FULLY PERMEABLE and only provides structural support.'},
        {q:'What is ONE advantage of using AI to identify cancer cells compared to manual inspection?',opts:['AI can create new cells','AI can identify cancer cells with greater speed and accuracy','AI can cure cancer','AI can replace the need for microscopes'],ans:1,skill:'Cells',fb:'AI systems can correctly identify cancer cells with greater speed and accuracy, helping doctors diagnose the type of cancer a patient has.'},
        {q:'Root hair cells in plants have NO chloroplasts. Why not?',opts:['Root hair cells do not need energy','Roots are underground so no light reaches them making chloroplasts useless','Root hair cells are animal cells','Chloroplasts are too large to fit'],ans:1,skill:'Cells',fb:'Chloroplasts need light for photosynthesis. Since roots are underground and receive no light, chloroplasts would serve no purpose — this is cellular specialisation.'},
        {q:'Which sequence correctly shows how cells are organised in a multicellular organism from smallest to largest?',opts:['Organ to Cell to Tissue to System to Organism','Cell to Organ to Tissue to System to Organism','Cell to Tissue to Organ to System to Organism','System to Organ to Tissue to Cell to Organism'],ans:2,skill:'Cells',fb:'The correct hierarchy is Cell to Tissue to Organ to System to Organism. Cells of the same type form tissues, different tissues form organs, organs form systems, and systems form the organism.'}
      ],
      secB:[
        {stmt:'Chloroplasts carry out photosynthesis so ALL plant cells must have chloroplasts.',ans:false,reason:'Not all plant cells have chloroplasts. Root hair cells are underground and receive no light, so they have no chloroplasts. Cells can be specialised exceptions to the typical cell model.'},
        {stmt:'The cytoplasm is described as jelly-like and is the site where most chemical reactions in the cell take place.',ans:true,reason:'The cytoplasm is the jelly-like fluid filling the cell AND is the site where most chemical reactions occur. Both parts of this statement are correct.'},
        {stmt:'An electron microscope and a light microscope can both be used to see all cell structures with equal clarity.',ans:false,reason:'An electron microscope provides MUCH higher magnification and resolution than a light microscope. Structures that cannot be seen clearly under a light microscope can be observed in detail under an electron microscope.'},
        {stmt:'The Euglena organism challenges the original two-kingdom classification of living things as either plants or animals.',ans:true,reason:'Euglena has characteristics of BOTH plant cells (chloroplasts for photosynthesis) AND animal cells (can actively swim and absorb food). This led to a new classification system.'},
        {stmt:'Division of labour in a multicellular organism means that every cell performs every function needed by the organism.',ans:false,reason:'Division of labour means the OPPOSITE. Different specialised cells each perform SPECIFIC functions. Muscle cells contract, nerve cells conduct signals, absorbing cells absorb nutrients. No single cell does everything.'}
      ],
      secC:[
        {type:'str',q:'Figure A shows a typical animal cell and Figure B shows a typical plant cell.\\n(a) Name ONE structure found in Figure B but NOT in Figure A. [1]\\n(b) Name TWO structures found in BOTH Figure A and Figure B. [2]\\n(c) Explain why plant cells need chloroplasts but animal cells do not. [2]',marks:5,scheme:'(a) Any ONE of: cell wall, chloroplast, large central vacuole [1]\\n(b) Any TWO of: nucleus, cytoplasm, cell membrane, vacuole [2]\\n(c) Plant cells carry out photosynthesis [1] — chloroplasts contain chlorophyll to convert light energy into food. Animal cells cannot photosynthesise [1] — they obtain energy by consuming and digesting food.'},
        {type:'str',q:'A student says all cells are the same because they all have a nucleus, cytoplasm, and cell membrane.\\n(a) Give ONE piece of evidence that challenges this statement. [2]\\n(b) Explain what specialisation means in the context of cells. [2]',marks:4,scheme:'(a) Any valid example: Red blood cells do not have a nucleus [1] stating what is different about it [1]; OR root hair cells have no chloroplasts [1]; OR Euglena has features of both plant and animal cells [1]\\n(b) Specialisation means cells are modified or adapted [1] to perform a specific function more efficiently, even if they differ from the typical cell model [1].'},
        {type:'str',q:'Using the human digestive system as an example, explain the terms tissue, organ, and system. [3]',marks:3,scheme:'Tissue: cells of the same type performing the same function — e.g. muscle tissue in the stomach [1]\\nOrgan: different tissues working together for a specific function — e.g. the stomach has muscle tissue and lining tissue [1]\\nSystem: organs working together — e.g. the digestive system includes the stomach, small intestine and large intestine [1]'}
      ]
    },
    {
      num:35, title:'WA3 Booster — Particulate Matter Deep Dive (Ch.7)',
      secA:[
        {q:'In 1827, Robert Brown observed pollen grains moving randomly in water. What caused this movement?',opts:['Pollen grains are alive and move on their own','Currents in the water pushed the pollen','Invisible water particles bombarded the pollen grains from all sides','The pollen grains were attracted to each other'],ans:2,skill:'Particulate Matter',fb:'Einstein explained Brownian motion: pollen grains were bombarded by invisible water molecules from all sides. Since bombardment is uneven, pollen moves randomly — proving matter is made of tiny moving particles.'},
        {q:'A perfume bottle is opened at the front of a classroom. Which factor would make the smell reach the back FASTER?',opts:['Lower room temperature','Heavier perfume particles','Higher room temperature','Smaller room'],ans:2,skill:'Particulate Matter',fb:'Higher temperature means particles have more energy and move faster, so diffusion occurs more quickly. Temperature is a key factor affecting the rate of diffusion.'},
        {q:'Dry ice (solid CO2) disappears without any liquid forming. What is this process called?',opts:['Melting','Condensation','Evaporation','Sublimation'],ans:3,skill:'Particulate Matter',fb:'Sublimation is when a solid changes DIRECTLY into a gas without passing through the liquid state. Dry ice sublimates at room temperature.'},
        {q:'Using the particulate model, which statement correctly explains why liquids cannot be compressed?',opts:['Liquid particles have no energy','Liquid particles are closely packed with little space between them to be pushed closer','Liquid particles are in fixed positions','Liquid particles are too heavy to compress'],ans:1,skill:'Particulate Matter',fb:'Liquids cannot be compressed because particles are already closely packed with very little space — there is nowhere for them to go when force is applied. Gases CAN be compressed because they have large spaces between particles.'},
        {q:'A student heats liquid water at 20 degrees C until it all boils away. Which state change sequence is correct?',opts:['Solid to Liquid to Gas','Liquid to Gas only','Solid to Gas','Liquid to Gas with no solid involved'],ans:3,skill:'Particulate Matter',fb:'Starting from liquid water, heating causes liquid to gas (boiling). No solid is involved. So the only state change is liquid to gas.'},
        {q:'Which observation is evidence of Brownian motion in everyday life?',opts:['Water flowing downhill','Dust particles dancing in a beam of light from a projector','Ice melting in a warm room','Steam rising from a hot drink'],ans:1,skill:'Particulate Matter',fb:'Dust particles dancing in a beam of light move randomly because they are being bombarded by invisible air particles — this IS Brownian motion.'},
        {q:'Which is a known limitation of the particulate model?',opts:['It cannot explain why matter has mass','The particles in the model are shown as spheres but real particles may not be spherical','It cannot explain the three states of matter','The model has been disproved by modern science'],ans:1,skill:'Particulate Matter',fb:'The particulate model shows particles as spheres but real particles may not be spherical. The model also does not show constant random movement. These are known limitations — but the model is still very useful.'},
        {q:'A doctor uses DW-MRI to detect a stroke. What property of water does DW-MRI measure?',opts:['The temperature of water in brain tissue','The colour of water molecules','The diffusion patterns of water in body tissues','The number of water particles in the brain'],ans:2,skill:'Particulate Matter',fb:'Diffusion-weighted MRI detects changes in the DIFFUSION PATTERNS of water in body tissues. When a stroke occurs, affected tissue changes how water diffuses — showing up as a white area on the image.'}
      ],
      secB:[
        {stmt:'When a substance condenses from gas to liquid, particles gain energy and move faster.',ans:false,reason:'During condensation the OPPOSITE happens: particles LOSE energy and slow down. Forces of attraction pull particles closer together. Gaining energy and moving faster describes boiling or evaporation, not condensation.'},
        {stmt:'Diffusion is the net movement of particles from a region of lower concentration to a region of higher concentration.',ans:false,reason:'This is the OPPOSITE of the correct definition. Diffusion is the NET movement of particles from HIGHER concentration to LOWER concentration, continuing until concentrations are equal throughout.'},
        {stmt:'When ink is added to water, the ink particles eventually spread evenly throughout the water due to diffusion.',ans:true,reason:'Ink particles are in constant random motion. They move from high concentration where the ink was dropped to lower concentration in the rest of the water by diffusion, until the concentration is uniform.'},
        {stmt:'Conservation of mass means that the total mass of a substance stays the same during expansion because no particles are created or destroyed.',ans:true,reason:'During expansion or any state change, the NUMBER and SIZE of particles remain unchanged. Only the DISTANCES between particles change. Since mass depends on number and size of particles, mass is conserved.'},
        {stmt:'A gas has a definite volume because its particles are held together by very strong forces of attraction.',ans:false,reason:'A gas does NOT have a definite volume. Gas particles are held together by WEAK forces of attraction, are very far apart, and move rapidly — they fill any container. Strong forces of attraction describe SOLIDS.'}
      ],
      secC:[
        {type:'str',q:'(a) State the difference between boiling and condensation in terms of particle behaviour. [4]\\n(b) Explain what happens when a gas is cooled and becomes liquid. [3]',marks:7,scheme:'(a) Boiling: particles in liquid gain energy [1], move faster and further apart [1], overcome forces of attraction [1], escape as gas [1]. Condensation: particles in gas lose energy [1], slow down [1], pulled closer by forces of attraction [1], become liquid [1] — award 4 marks for 4 correct points.\\n(b) When a gas is cooled, particles lose energy [1]. They move more slowly [1]. Forces of attraction pull the particles closer together [1] until they become a liquid.'},
        {type:'str',q:'A student adds food colouring to cold water and hot water.\\n(a) In which beaker will diffusion occur faster? Give a reason. [2]\\n(b) After 30 minutes, what would you observe in both beakers? [1]\\n(c) State the definition of diffusion. [1]',marks:4,scheme:'(a) Hot water [1]; because particles in hot water have more energy and move faster, so diffusion occurs at a faster rate [1]\\n(b) In both beakers, the colour would be evenly and uniformly distributed throughout the water [1]\\n(c) Diffusion is the net movement of particles from a region of higher concentration to a region of lower concentration [1]'},
        {type:'str',q:'Explain how the particulate nature of matter explains why: (a) a gas fills a container completely [2], (b) a solid keeps its shape [2], (c) a liquid flows but cannot be compressed [2].',marks:6,scheme:'(a) Gas particles are very far apart and move rapidly and randomly in all directions [1], spreading out to fill the entire volume of any container [1]\\n(b) Solid particles are very closely packed in an orderly arrangement and vibrate about fixed positions [1], so the solid maintains a definite fixed shape [1]\\n(c) Liquid particles are closely packed so cannot be compressed as there is no space to push closer [1] but they move randomly and slide past one another so can flow [1]'}
      ]
    },
    {
      num:36, title:'WA3 Final Challenge — Exam Simulation (Ch.6 and Ch.7)',
      secA:[
        {q:'Which row correctly shows ALL properties of a liquid?',opts:['Definite volume Yes and Definite shape Yes and Can be compressed No','Definite volume Yes and Definite shape No and Can be compressed No','Definite volume No and Definite shape No and Can be compressed Yes','Definite volume No and Definite shape Yes and Can be compressed No'],ans:1,skill:'Particulate Matter',fb:'A liquid has DEFINITE volume, has NO definite shape (takes shape of container), and CANNOT be compressed (particles already closely packed). Option B matches all three.'},
        {q:'A student adds red dye to water. Which action would NOT speed up diffusion?',opts:['Warming the water','Stirring the water','Using smaller lighter dye particles','Cooling the water'],ans:3,skill:'Particulate Matter',fb:'Cooling the water would SLOW diffusion — colder particles have less energy and move more slowly. Warming, stirring, and using smaller particles all increase diffusion rate.'},
        {q:'Why does a typical plant cell have a more regular rectangular shape than an animal cell?',opts:['Plant cells are larger','The cell wall is rigid and gives the plant cell a fixed regular shape','Plant cells have more cytoplasm','The nucleus in plant cells is larger'],ans:1,skill:'Cells',fb:'The cell wall is rigid (tough cellulose) and gives the plant cell its fixed regular rectangular shape. Animal cells lack a cell wall so have irregular flexible shapes.'},
        {q:'A student observes: particles lose energy, slow down, and are pulled from a close but disorderly arrangement into fixed positions. Which state change is this?',opts:['Melting','Boiling','Freezing','Sublimation'],ans:2,skill:'Particulate Matter',fb:'This describes FREEZING (liquid to solid): particles lose energy, slow down, and are pulled by forces of attraction into fixed orderly positions. Close but disorderly means liquid and fixed positions means solid.'},
        {q:'What is ONE advantage of being multicellular rather than unicellular?',opts:['Multicellular organisms are always larger','Cells can be specialised for different functions allowing division of labour and more efficient functioning','Multicellular organisms have more organelles per cell','Multicellular organisms do not need food'],ans:1,skill:'Cells',fb:'The key advantage is division of labour — cells can be specialised for specific functions, allowing the organism to function much more efficiently than if every cell had to do everything.'},
        {q:'Which concept from Chapter 7 explains how virus particles spread to people in a room after someone coughs?',opts:['Expansion and contraction','Conservation of mass','Diffusion and Brownian motion','State changes'],ans:2,skill:'Particulate Matter',fb:'Virus particles suspended in air exhibit Brownian motion — they move randomly due to collisions with air particles. Through diffusion they spread from high concentration near the infected person to lower concentration in the rest of the room.'},
        {q:'Which correctly shows the THREE structures unique to plant cells not found in typical animal cells?',opts:['Nucleus, cytoplasm, cell membrane','Cell wall, chloroplast, large central vacuole','Nucleus, chloroplast, vacuole','Cell wall, nucleus, cytoplasm'],ans:1,skill:'Cells',fb:'The THREE structures unique to plant cells are: cell wall providing rigid structural support, chloroplast for photosynthesis, and large central vacuole storing water and food and waste. Nucleus, cytoplasm, and cell membrane are found in BOTH plant and animal cells.'},
        {q:'Scientists use computer modelling and AI to track movements of particles in matter. What does this tell us about the particulate model?',opts:['The model is completely accurate and needs no improvement','Scientists have abandoned the particulate model','The model is still being improved as technology advances','AI has replaced all need for particle models'],ans:2,skill:'Particulate Matter',fb:'With advancements in technology, scientists use computer modelling to visualise particle vibrations more accurately. Science is an ongoing endeavour — models are continuously improved.'}
      ],
      secB:[
        {stmt:'A solid cannot flow because its particles vibrate about fixed positions and cannot slide past each other.',ans:true,reason:'Solid particles are very closely packed in an orderly arrangement and vibrate about fixed positions — they cannot slide past each other, which is why solids cannot flow. Liquids can flow because their particles CAN slide past one another.'},
        {stmt:'A cell wall is found in plant cells and performs the same function as the cell membrane — controlling what enters and leaves the cell.',ans:false,reason:'The cell WALL and cell MEMBRANE have DIFFERENT functions. The cell wall is FULLY PERMEABLE and provides rigid structural support and shape. Only the cell MEMBRANE is partially permeable and controls movement of substances into and out of the cell.'},
        {stmt:'When ice melts, the water produced has less mass than the original ice because some particles are used up during melting.',ans:false,reason:'Mass is CONSERVED during melting and all state changes. The SAME number of particles are present — no particles are created or destroyed. The water produced has EXACTLY the same mass as the original ice.'},
        {stmt:'Bioengineers can use knowledge of cells to 3D print artificial organs such as hearts and livers for patients who need transplants.',ans:true,reason:'Bioengineers apply knowledge of living cells and scientific principles to create artificial body parts and organs through 3D printing. Patients who need replacement organs can receive them without waiting for a suitable donor.'},
        {stmt:'The particulate model perfectly represents the constituents of matter and has no limitations.',ans:false,reason:'The particulate model has limitations — real particles may not be spherical as shown in the model, and the model does not show constant random movement of particles. Scientists use computer simulations to develop more accurate models.'}
      ],
      secC:[
        {type:'str',q:'A nurse wears a mask when treating an infectious patient.\\n(a) Using diffusion and Brownian motion, explain why the mask helps reduce the risk of infection. [4]\\n(b) State ONE way knowledge of diffusion has been applied in medical imaging. [2]',marks:6,scheme:'(a) When the patient coughs, virus particles are released into the air [1]. These particles undergo Brownian motion and move randomly due to bombardment by air particles [1]. Through diffusion, virus particles spread from high concentration near the patient to lower concentration [1]. A mask prevents most virus particles from being dispersed into the air or reaching the nurse [1].\\n(b) Diffusion-weighted MRI (DW-MRI) uses diffusion patterns of water in body tissues to detect changes caused by diseases such as stroke or cancer [1]. It allows doctors to identify the stage of disease and monitor responses to treatment [1].'},
        {type:'str',q:'A student examines an unknown cell and observes: a definite rectangular shape, a large space in the centre filled with fluid, and structures that appear green under the microscope.\\n(a) Identify the type of cell and give TWO reasons. [3]\\n(b) Name the green structures and state their function. [2]\\n(c) Name the structure that gives this cell its rectangular shape. [1]',marks:6,scheme:'(a) Plant cell [1]; because it has a regular rectangular shape due to cell wall [1]; AND/OR large central vacuole [1]; AND/OR green structures (chloroplasts) found only in plant cells [1] — award max 2 marks for reasons.\\n(b) Chloroplasts [1]; carry out photosynthesis and contain chlorophyll allowing the plant to convert light energy into food/glucose [1].\\n(c) Cell wall [1]'},
        {type:'str',q:'Compare the particulate arrangement in a solid and a liquid. Address: (i) arrangement of particles, (ii) movement of particles, (iii) forces of attraction, (iv) definite volume and shape. [8]',marks:8,scheme:'Arrangement: Solid — very closely packed in orderly/regular arrangement [1]; Liquid — closely packed but in disorderly/random arrangement [1]\\nMovement: Solid — particles vibrate about fixed positions [1]; Liquid — particles move randomly and slide past one another [1]\\nForces: Solid — held together by very strong forces of attraction [1]; Liquid — held together by strong forces of attraction (weaker than solid) [1]\\nVolume/Shape: Solid — definite volume AND definite shape [1]; Liquid — definite volume but NO definite shape takes shape of container [1]'}
      ]
    }
